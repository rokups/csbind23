...
int main(int argc, char** argv)
{
    ...
    const std::filesystem::path default_output_root =
        std::filesystem::path(CSBIND23_SOURCE_DIR) / "example" / "generated";
#else
    const std::filesystem::path default_output_root = std::filesystem::current_path() / "example" / "generated";
#endif

    const std::filesystem::path output_root = argc > 1 ? std::filesystem::path(argv[1]) : default_output_root;
