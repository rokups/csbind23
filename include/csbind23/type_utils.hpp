...
template <typename Type> std::string qualified_type_name()
{
...
template <typename Type> std::string unqualified_type_name()
{
    return unqualified_name(qualified_type_name<Type>());
}

template <auto Function> std::string function_symbol_name()
{
#if defined(__clang__) || defined(__GNUC__)
    constexpr std::string_view pretty = __PRETTY_FUNCTION__;
...
template <auto Function> std::string function_export_name()
{
...
template <typename MethodPtr> struct MethodPointerTraits;
