...
ModuleDecl& BindingsGenerator::upsert_module(std::string_view name) {
    const auto existing_it = std::find_if(modules_.begin(), modules_.end(), [name](const ModuleDecl& module_decl) {
        return module_decl.name == name;
    });
...
ModuleBuilder::ModuleBuilder(BindingsGenerator& owner, ModuleDecl& module_decl)
    : owner_(&owner), module_decl_(&module_decl) {
}
