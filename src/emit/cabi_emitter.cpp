...
std::string render_argument_expr(const ParameterDecl& parameter) {
    return std::format("csbind23::cabi::Converter<{}>::from_c_abi({})", render_cpp_type(parameter.type), parameter.name);
}
...
void append_free_function_body(TextWriter& output, const FunctionDecl& function_decl) {
    ...
    output.append_line_format(
        "    return csbind23::cabi::Converter<{}>::to_c_abi(result);",
        render_cpp_type(function_decl.return_type)
    );
}
...
void append_constructor_body(TextWriter& output, const ClassDecl& class_decl, const FunctionDecl& ctor_decl) {
    ...
    output.append_line_format(
        "    return csbind23::cabi::Converter<{}*>::to_c_abi(instance);",
        class_decl.cpp_name
    );
}
...
void append_method_body(TextWriter& output, const ClassDecl& class_decl, const FunctionDecl& method_decl) {
    if (method_decl.is_const) {
        output.append_line_format(
            "    auto* instance = csbind23::cabi::Converter<const {}*>::from_c_abi(self);",
            class_decl.cpp_name
        );
    } else {
        output.append_line_format(
            "    auto* instance = csbind23::cabi::Converter<{}*>::from_c_abi(self);",
            class_decl.cpp_name
        );
    }
...
std::vector<std::filesystem::path> emit_cabi_module(const ModuleDecl& module_decl, const std::filesystem::path& output_root) {
    ...
    for (const auto& class_decl : module_decl.classes) {
        ...
        if (emits_owned_instance) {
            generated.append_line_format("extern \"C\" void {}_{}_destroy(void* self) {{", module_decl.name, class_decl.name);
            generated.append_line_format("    delete csbind23::cabi::Converter<{}*>::from_c_abi(self);", class_decl.cpp_name);
            generated.append_line("}");
            generated.append_line();
        }
