...
std::filesystem::path write_csharp_file(const std::filesystem::path& output_root, const std::string& filename, const std::string& content) {
...
void append_native_signature(TextWriter& output, const FunctionDecl& function_decl, const std::string& exported_name, bool include_self) {
    output.append_line("    [System.Runtime.InteropServices.DllImport(\"csbind23_native\", CallingConvention = System.Runtime.InteropServices.CallingConvention.Cdecl)]");
...
void append_wrapper_method(TextWriter& output, const std::string& module_name, const ClassDecl& class_decl, const FunctionDecl& method_decl) {
...
void append_wrapper_class(TextWriter& output, const std::string& module_name, const ClassDecl& class_decl) {
    ...
    output.append_line("        {");
    if (emits_destroy) {
        output.append_line_format("            {}Native.{}_{}_destroy(_handle);", module_name, module_name, class_decl.name);
    }
    output.append_line("        }");
    output.append_line();
...
std::vector<std::filesystem::path> emit_csharp_module(const ModuleDecl& module_decl, const std::filesystem::path& output_root) {
    ...
    for (const auto& function_decl : module_decl.functions) {
        append_native_signature(generated, function_decl, module_decl.name + "_" + function_decl.name, false);
    }
    ...
    for (const auto& class_decl : module_decl.classes) {
        ...
        for (const auto& method_decl : class_decl.methods) {
            if (method_decl.is_constructor) {
                ...
                append_native_signature(generated, method_decl, module_decl.name + "_" + class_decl.name + "_" + method_decl.name, true);
            }
        }
    }
